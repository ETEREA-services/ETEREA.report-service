...
def get_default_branch(wiki_url, headers):
    """Determine the default branch of the wiki"""
    try:
    ...
    try:
        ...
        if check and result.returncode != 0:
            if 'Authentication failed' in result.stderr:
                raise WikiGenerationError("Git authentication failed. Please verify the token.")
            elif 'repository not found' in result.stderr:
                raise WikiGenerationError("Wiki repository not found. Please verify it's enabled.")
            elif error_msg:
                raise WikiGenerationError(f"{error_msg}: {result.stderr}")
            else:
                raise WikiGenerationError(f"Git command error: {result.stderr}")
        return result
    except Exception as e:
        if not isinstance(e, WikiGenerationError):
            raise WikiGenerationError(f"Git execution error: {str(e)}")
        raise

def verify_token():
    """Verify token existence and format"""
    token = os.environ.get('GITHUB_TOKEN')
    if not token:
        raise WikiGenerationError("GITHUB_TOKEN not found in environment variables")
    if not token.strip():
        raise WikiGenerationError("GITHUB_TOKEN is empty")
    return token
...
""")
        ...
        with open(wiki_dir / 'Milestones.md', 'w', encoding='utf-8') as f:
            f.write("# Milestones del Servicio Core\n\n")
        ...
        with open(wiki_dir / 'Issues-Activos.md', 'w', encoding='utf-8') as f:
            f.write("# Issues Activos - Servicio Core\n\n")
        ...
        with open(wiki_dir / 'Issues-Cerrados.md', 'w', encoding='utf-8') as f:
            f.write("# Issues Cerrados - Servicio Core\n\n")
        ...
        print(f"Error escribiendo archivos de la wiki del Servicio Core: {e}")
        return False
